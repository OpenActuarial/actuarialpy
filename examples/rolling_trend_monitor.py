...
12-month window to the same window one year earlier, and ``decompose_per_exposure_trend``
reports both the observed year-over-year trend and whether it is frequency- or
...
from actuarialpy import decompose_per_exposure_trend  # noqa: E402


def build_monthly_panel() -> pd.DataFrame:
    """A monthly incurred panel: real seasonality, a 2024 frequency acceleration,
...
def main() -> None:
    ...
    for eval_month in pd.period_range("2024-01", "2024-12", freq="M"):
        ...
        d = decompose_per_exposure_trend(prior, current, **cols).iloc[0]

        # contrast: single-month year-over-year (same calendar month, one month each)
        this_cost = df.loc[df["month"] == eval_month, "allowed"].iloc[0] / df.loc[df["month"] == eval_month, "member_months"].iloc[0]
        year_ago_cost = df.loc[df["month"] == eval_month - 12, "allowed"].iloc[0] / df.loc[df["month"] == eval_month - 12, "member_months"].iloc[0]

        records.append({
            "eval_month": str(eval_month),
            "r12_cost": d["loss_per_exposure_current"],
            "r12_yoy": d["loss_per_exposure_trend"] - 1,        # the monitor
            "freq": d["frequency_trend"] - 1,           # ... split into its drivers
            "sev": d["severity_trend"] - 1,
            "1mo_yoy": this_cost / year_ago_cost - 1,
        })

    out = pd.DataFrame(records)
    print("Rolling-12 observed trend, refreshed monthly (each row compares two annual windows)\n")
    show = out.copy()
    show["r12_cost"] = show["r12_cost"].map(lambda v: f"{v:,.0f}")
    for c in ("r12_yoy", "freq", "sev", "1mo_yoy"):
        show[c] = show[c].map(lambda v: f"{v:+.1%}")
    print(show.to_string(index=False))

    print("\nReading it:")
    print("- r12_yoy is the monitor: it climbs ~8% -> ~12% across 2024. Because a trailing")
    print("  average ramps a change in slowly, the frequency acceleration that began in")
    print("  January only reads ~+3.8% by December -- the signal is real but lagged.")
    print("- The frequency/severity split separates the story: frequency drifting up is broad-based,")
